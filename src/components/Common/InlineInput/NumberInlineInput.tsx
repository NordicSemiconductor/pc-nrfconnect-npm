...
import { isFactor } from '../Slider/Slider'
...
const incrementValue = (current: number, range: RangeProp, steps: number, action: ((v: number) => void)) => {
    if (steps === 0) 
        return;

    if (typeof range.explicitRange != 'undefined' && range.explicitRange.length > 0) {
    ...
    const decimal = (range.decimals && typeof range.decimals != 'undefined')? range.decimals : 0;
    const stepValue =  ((range.step && typeof range.step != 'undefined')? range.step : 0.1 ** decimal)
...
const NumberInlineInput: FC<Props> = ({
    ...
    <InlineInput
        ...
        onKeyboardIncrementAction={ () => incrementValue(value, range, 1, onChange)}
        onKeyboardDecrementAction={ () => incrementValue(value, range, -1, onChange)}
    />
);
